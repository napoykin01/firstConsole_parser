...
def fetch_products(catalog_name: str,
                   category_id: int, token: str) -> List[Dict[str, Any]]:
    ...
    goods_list = []
    goods = root.findall('.//ns:goods', namespaces=ns)

    for good in goods:
        good_id = good.find('ns:id', namespaces=ns).text

        properties = {}
        props = good.findall('.//ns:property', namespaces=ns)
        for prop in props:
...
@router.get("/products/{catalog_name}/{category_id}",
            summary="Получить список товаров категории")
def get_products(catalog_name: str,
                 category_id: int,
                 token: str = Depends(get_current_token)) -> List[Dict[str, Any]]:
    return fetch_products(catalog_name, category_id, token)