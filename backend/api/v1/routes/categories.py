...
async def fetch_categories(catalog_name: str, token: str) -> List[Dict[str, Any]]:
    ...
    if response.status_code != 200:
        raise HTTPException(
            status_code=400,
            detail=f"Failed to fetch categories for catalog '{catalog_name}'",
        )
    ...
    for cat in root.findall(".//ns:category", namespaces=ns):
...
async def get_categories(
    ...
    """Возвращает список категорий по имени каталога (id, name, parentId, leaf)."""
    return await fetch_categories(catalog_name, token)